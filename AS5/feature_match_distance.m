...
for j = 1:col
    if(x2(j)<win_half+2||x2(j)>r-win_half-2||y2(j)<win_half+2||...
            y2(j)>c-win_half-2)
        window_y_all=[window_y_all zeros(win_size,win_size)];
        continue;
    end;  
    x2_ceil=ceil(x2(j));
    y2_ceil=ceil(y2(j));
    ...
    window_y_all=[window_y_all window_y];
end;
...
for i = 1:row
    ...
    corr_all=normxcorr2(window_x,window_y_all);
    for j=1:col
        cor(i,j)=corr_all(win_size,win_size*j);
    end;
end;
