function [ matches ] = feature_match( im1 ,im2 ,x1 , y1 , x2 , y2 ,...
    win_size , cor_thre , dis_thre)
...
for i = 1:row 
    if(x1(i)<win_half+1||x1(i)>r-win_half-1||y1(i)<win_half+1||...
            y1(i)>c-win_half-1)
        continue;
    end;
    x_ceil=ceil(x1(i));
    ...
    for j = 1:col
        if(x2(j)<win_half+1||x2(j)>r-win_half-1||y2(j)<win_half+1||...
                y2(j)>c-win_half-1)
            continue;
        end;
        x2_ceil=ceil(x2(j));
