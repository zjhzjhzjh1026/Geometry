...
matches = feature_match_distance( im1_grey ,im2_grey ,x1_corner ,...
    y1_corner , x2_corner , y2_corner , window_size, cor_thre ,dis_thre);

len3=size(matches,1);
rr=ones(len3,1)*window_size;

figure(4);
im_square11 = insertShape(im1,'rectangle',[matches(:,2)-fix(...
    window_size/2) matches(:,1)-fix(window_size/2) rr rr],'LineWidth',1);
...
im_square22 = insertShape(im2,'rectangle',[matches(:,4)-fix(...
    window_size/2) matches(:,3)-fix(window_size/2) rr rr],'LineWidth',1);
